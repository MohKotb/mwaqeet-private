...
// ========== إعدادات التحديث الشامل من GitHub ==========
var GITHUB_USER = "MohKotb";                 // اسم المستخدم الخاص بك
var GITHUB_REPO = "mwaqeet-private";         // اسم المستودع
var GITHUB_BRANCH = "main";                  // الفرع الرئيسي
var GITHUB_TOKEN = "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";   // التوكن الذي حصلت عليه (انسخه هنا)
var LOCAL_PATH = "/mnt/sdcard/mwaqeet-private/";     // مجلد الساعة على الجهاز

// رابط ملف رقم الإصدار
var VERSION_URL = "https://raw.githubusercontent.com/" + GITHUB_USER + "/" + GITHUB_REPO + "/" + GITHUB_BRANCH + "/version.txt?token=" + GITHUB_TOKEN;

// ✏️ ضع هنا جميع ملفات المشروع التي تريد تحميلها عند التحديث
var ALL_PROJECT_FILES = [
  ...
  "image/athkar.jpg",
  "image/fasting_monday.jpg",
  "image/fasting_thursday.jpg",
  "image/fasting_white_days.jpg",
  "data/CAIRO2.js",
  "data/CAIRO.js",
  ...
  "data/HURGHADA.js",
  "data/ISMAILIA.js",
  "data/KAFRADDAWWAR.js",
  "data/KAFRASHSHAYKH.js",
  "data/LUXOR.js",
  "data/MERSAMATRUH.js",
  "data/PORT-SAID.js",
  "data/QINA.js",
  "data/SHARMELSHEIKH.js",
  "data/SOHAG.js",
  "data/SUEZ.js",
  "data/ZAGAZIG.js",
  "data/ZAGAZIG2.js"
  // ... أضف كل ملف صوتي وصورة ومحافظة لديك بهذا الشكل، كل ملف بين علامتي تنصيص ثم فاصلة
];
...
async function checkForUpdates() {
  ...
  try {
    ...
    if (!response.ok) throw new Error("فشل جلب الإصدار");
    var latestVersion = (await response.text()).trim();
    var currentVersion = localStorage.getItem('app_version') || "0.0";

    if (latestVersion !== currentVersion) {
      if (!confirm("يوجد إصدار جديد (" + latestVersion + "). هل تريد تحميل جميع الملفات الآن؟")) return;
      setStatus("جاري تحميل ملفات المشروع...");

      // تنزيل كل ملف في القائمة واستبداله
      for (var i = 0; i < ALL_PROJECT_FILES.length; i++) {
        var filePath = ALL_PROJECT_FILES[i];
        var remoteUrl = "https://raw.githubusercontent.com/" + GITHUB_USER + "/" + GITHUB_REPO + "/" + GITHUB_BRANCH + "/" + filePath + "?token=" + GITHUB_TOKEN;
        var localFile = LOCAL_PATH + filePath;
        setStatus("ينزل: " + filePath);
        await downloadAndSave(remoteUrl, localFile);
      }

      localStorage.setItem('app_version', latestVersion);
      alert("تم التحديث بنجاح! سيتم إعادة التشغيل الآن.");
      window.location.reload(true);
    } else {
      setStatus("نسختك هي الأحدث (" + currentVersion + ")");
      alert("نسختك هي الأحدث.");
    }
  } catch (error) {
    setStatus("فشل التحديث: " + error.message);
    alert("فشل التحديث. تأكد من الاتصال ومن صحة التوكن والروابط.");
  }
}

function downloadAndSave(url, destinationPath) {
  return new Promise(function (resolve, reject) {
    if (typeof fully !== 'undefined' && fully.downloadFile) {
      fully.downloadFile(url, destinationPath, function (success) {
        if (success) resolve();
        else reject(new Error("لم يتم حفظ " + destinationPath));
      });
    } else {
      reject(new Error("جهازك لا يدعم الوصول للملفات."));
    }
  });
}